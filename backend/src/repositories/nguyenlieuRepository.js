...
const NguyenLieuRepository = {
  ...
  importGoods: async (data) => {
    ...
    try {
      ...
      ngayNhap.setHours(ngayNhap.getHours() + 7);
      ...
      if (data.items && data.items.length > 0) {
        for (const item of data.items) {
          ...
          await conn.execute(
            `UPDATE nguyenlieu 
             SET ml_thuc_te_ton = ml_thuc_te_ton + IF(danh_muc IN ('Nguyên liệu pha chế', 'Sản phẩm hết trong ngày'), ( ? * dung_tich_san_pham ), ?) 
      ...
      SELECT 
        ...
        JSON_ARRAYAGG(
          JSON_OBJECT(
            ...
            'don_vi_tinh', IF(nl.danh_muc IN ('Nguyên liệu pha chế', 'Sản phẩm hết trong ngày'), nl.don_vi_tinh, nl.don_vi_nhap),
  ...
  // ⚡ TỰ ĐỘNG RESET: Nhóm 'Sản phẩm hết trong ngày' tự động reset về 0 nếu phát hiện bước sang ngày mới
  getAll: async () => {
    const conn = await db.getConnection();
    try {
      await conn.execute(
        `UPDATE nguyenlieu 
         SET ml_thuc_te_ton = 0.00 
         WHERE danh_muc = 'Sản phẩm hết trong ngày' 
           AND ma_nguyen_lieu IN (
             ...
             WHERE DATE(p.ngay_nhap) < CURDATE()
           )`
      );
      ...
      const sql = `
        SELECT 
          ...
          CAST(ml_thuc_te_ton AS DECIMAL(10,2)) AS ml_thuc_te_ton, 
  ...
  create: async (data) => {
    ...
    const [result] = await db.execute(
      `INSERT INTO nguyenlieu (ten_nguyen_lieu, danh_muc, don_vi_tinh, don_vi_nhap, don_vi_dong_goi, dung_tich_san_pham, ml_thuc_te_ton, nguong_canh_bao, ghi_chu, trang_thai) VALUES (?, ?, ?, ?, ?, ?, 0.00, ?, ?, 1)`,
